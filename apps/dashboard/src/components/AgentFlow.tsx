...
const CustomAgentNode = ({ data }: { data: any }) => {
  ...
  return (
    <div className={`relative flex items-center h-[52px] min-w-[190px] rounded-lg border text-left transition-all duration-300 shadow-lg custom-agent-node ${borderStyle}`}>
      {/* Input Handle */}
      {label !== "Scanner" && (
        <Handle 
          type="target" 
      ...
      <div className="flex flex-1 items-center justify-between gap-2 px-3">
        <div className="flex items-center gap-2">
          <div className="flex h-6 w-6 items-center justify-center rounded bg-[#150507] border border-[#4e1c22]/40 custom-agent-node-icon">
            {Icon && <Icon className={`h-3.5 w-3.5 ${active ? "text-[#cc4155]" : completed ? "text-emerald-400" : "text-[#9d7d82]/60"}`} />}
          </div>
          <div className="flex flex-col">
            <span className="text-[10px] text-white font-bold leading-tight custom-agent-node-label">{label}</span>
            <span className="text-[7px] text-[#9d7d82] font-semibold uppercase tracking-wider custom-agent-node-desc">{agentName.split(" ")[0]}</span>
          </div>
        </div>
